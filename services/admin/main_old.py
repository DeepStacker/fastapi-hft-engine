...
# SECURITY FIX: Removed /database/query endpoint due to SQL injection risk
# Users should use the safe /database/stats endpoint or connect with read-only credentials
...
async def startup():
    ...
    global redis_pubsub_client
    settings = get_settings()
    redis_pubsub_client = await redis.from_url(settings.REDIS_URL, decode_responses=True)
