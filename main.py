import asyncio
import sys
from fastapi import FastAPI, WebSocket, Depends, HTTPException, WebSocketDisconnect, Query
...
async def get_data_statistics(db=Depends(get_db)):
    ...
    async with db as session:
        # Get option stats
        option_count = await session.execute(select(func.count()).select_from(OptionContract))
        ...
        # Get futures stats
        future_count = await session.execute(select(func.count()).select_from(FutureContract))
        ...
        # Get market stats
        market_count = await session.execute(select(func.count()).select_from(MarketSnapshot))
