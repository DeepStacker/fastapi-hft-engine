...
async def get_option_chain(
    ...
    strike_count: Optional[int] = Query(..., description="Number of strikes around ATM to return", ge=2, le=100) # Added strike_count as Query param
):
    ...
    if symbol_exp is None:
        try:
            ...
            if (symbol_expiry_data and
                isinstance(symbol_expiry_data.get("data"), dict) and
                isinstance(symbol_expiry_data["data"].get("opsum"), dict) and
                symbol_expiry_data["data"]["opsum"]):
                ...
                logger.error(f"Failed to fetch or parse valid expiry data for {symbol_seg}:{symbol_sid}. Response: {symbol_expiry_data}")
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Could not determine expiry date for the symbol.")
        except OptionChainError as e:
             logger.error(f"Error fetching expiry dates for auto-detection: {e}")
             raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Failed to fetch expiry data: {e}")
        except Exception as e:
             logger.error(f"Unexpected error during expiry auto-detection: {e}", exc_info=True)
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error auto-detecting expiry date.")
    ...
    try:
        ...
        if cached_data:
            # Apply strike_count filtering even to cached data if necessary
            # This depends on whether the cache should store full or filtered data
            # For simplicity here, we assume cache stores full data and filter on retrieval
            if len(cached_data.get("data", [])) > strike_count:
                 # This assumes cached 'data' is the list of strike records
                 # Adjust if cached structure is different
                #  cached_data["data"] = cached_data["data"][:strike_count]
        ...
        # Filter the number of strikes based on strike_count parameter
        # TODO: Implement logic to select strikes *around* ATM if needed,
        # current implementation just takes the first N strikes.
        # This might require knowing the spot price from market_data first.
        # if len(records) > strike_count:
        #     # Simple truncation - enhance if ATM-centric filtering is required
        #     records = records[:strike_count]
        ...
        keys_to_keep_fut = { # Define keys to keep instead of keys to remove
            "ltp", "oichng", "oi", "vol", "bid", "ask", "bid_qty", "ask_qty",
            "iv", "delta", "theta", "gamma", "vega", "rho", "theoryprc", "mness"
            # Add any other required future keys here
        }
        fut_data = {}
        if isinstance(raw_fut_data, dict):
            for fut_key, fut_details in raw_fut_data.items():
                 if isinstance(fut_details, dict):
                     fut_data[fut_key] = {k: fut_details[k] for k in keys_to_keep_fut if k in fut_details}
        ...
        # Cache the potentially *full* data before filtering? Or cache filtered?
        # Caching full data allows future requests with different strike_count to hit cache.
        # Caching filtered data saves cache space but reduces hit rate for different counts.
        # Let's assume we cache the *full* response structure *before* filtering 'records'
        # This requires constructing the full response first, then filtering for the return value.
        # Re-evaluating: Caching the final filtered response is simpler.
    ...
    try:
        ...
        if (
            ...
            and isinstance(data["data"].get("opsum"), dict)
             ...
             expiry_data = [
                 value.get("exp", 0)
                 for value in data["data"]["opsum"].values() if value.get("exp") is not None
             ]
        ...
        response_data = {
            ...
            "metadata": {
                "symbol_seg": symbol_seg,
                "symbol_sid": symbol_sid,
                "curr_exp": expiry_data[0] if expiry_data else None, # Nearest expiry after sort
